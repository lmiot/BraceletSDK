 ...
 * 总步数（天）
 */
public class PaceDayFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(R.layout.fragment_pace_day, null);
        initView(view);
        return view;
    }
    @Override
    ...
    private void initView(View view) {
        ...
        columnar_chart =(LinearLayout )view.findViewById(R.id.PaceDayFragment_columnar_chart  ) ;
    ...
    private XYMultipleSeriesDataset getDataset() {
        ...
        //读取本地数据到曲线上
        String currentb6=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-6))).getDay_step();
        series .add(1, Double.parseDouble(currentb6));
        String currentb5=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-5))).getDay_step();
        series .add(2, Double.parseDouble(currentb5));
        String currentb4=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-4))).getDay_step();
        series .add(3, Double.parseDouble(currentb4));
        String currentb3=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-3))).getDay_step();
        series .add(4, Double.parseDouble(currentb3));
        String currentb2=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-2))).getDay_step();
        series .add(5, Double.parseDouble(currentb2));
        String currentb1=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(-1))).getDay_step();
        series .add(6, Double.parseDouble(currentb1));
        String currentDay=(manager.findSportDataInfo(SPUtil.getUserName(getContext()),
                TimeUtils.getdayDate_otherFormat(0))).getDay_step();
        LogUtils.d("currentDATA",TimeUtils.getdayDate_otherFormat(0));
        series .add(7, Double.parseDouble(currentDay));
        Ymax= math.getMax( Double.parseDouble(currentb6), Double.parseDouble(currentb5), Double.parseDouble(currentb4),
                Double.parseDouble(currentb3), Double.parseDouble(currentb2), Double.parseDouble(currentb1),
                Double.parseDouble(currentDay));
        // 把添加了点的折线放入dataset
    ...
    private void setChartSettings(XYMultipleSeriesRenderer renderer) {
        ...
        renderer.addYTextLabel(10000, "1W");
        renderer.addYTextLabel(20000, "2W");
        renderer.addYTextLabel(30000, "3W");
