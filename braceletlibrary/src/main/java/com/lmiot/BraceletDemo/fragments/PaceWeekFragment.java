 ...
 * 总步数（周）
 */
public class PaceWeekFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(R.layout.fragment_pace_week, null);
        initView(view);
        return view;
    }
    @Override
    ...
    private void initView(View view) {
        columnar_chart =(LinearLayout )view.findViewById(R.id.PaceWeekFragment_columnar_chart  ) ;
    ...
    private int adding(int week) {
        ...
        if(sportDataInfos!=null) {
            for (SportDataInfo s : sportDataInfos) {
                ...
                sum = sum + Integer.parseInt(s.getDay_step());

                LogUtils.d("listsport001",sportDataInfos.size()+":"+s.getDay_step());
            }
            return sum;
        }
        return  0;
